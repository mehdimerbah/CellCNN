...
def train_model(train_samples, train_phenotypes, outdir,
    ...
    for irun in range(nrun):
        ...
        # Ensure k is an integer and not None
        assert isinstance(k, int) and k > 0, f"Invalid value for k: {k}"
        print(f"Run {irun + 1}: maxpool_percentage={mp}, k={k}")
...
def build_model(ncell, nmark, nfilter, coeff_l1, coeff_l2,
    ...
    # Replace the Lambda layer with the custom PoolTopKLayer
    pooled = PoolTopKLayer(k)(conv)
...
def pool_top_k(x, k):
    return tf.reduce_mean(
        tf.sort(x, axis=1, direction='DESCENDING')[:, :k, :], axis=1
    )

class PoolTopKLayer(tf.keras.layers.Layer):
    def __init__(self, k, **kwargs):
        super(PoolTopKLayer, self).__init__(**kwargs)
        self.k = k

    def call(self, inputs, **kwargs):
        # Perform the top-k pooling operation
        sorted_inputs = tf.sort(inputs, axis=1, direction='DESCENDING')
        top_k_inputs = sorted_inputs[:, :self.k, :]
        pooled = tf.reduce_mean(top_k_inputs, axis=1)
        return pooled

    def compute_output_shape(self, input_shape):
        # Define the output shape as (batch_size, nfilter)
