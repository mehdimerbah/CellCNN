...
| `--nrun`   | Specifies the number of neural network configurations to try during training (default: `15`). Each run utilizes a different combination of hyperparameters such as the number of filters (`nfilter`), learning rate (`learning_rate`), and max-pooling percentage (`maxpool_percentage`). This exploration of various configurations allows the model to identify the most effective hyperparameter settings for the given dataset. After completing all runs, the three best-performing models based on validation accuracy are selected for further analysis and deployment. |
